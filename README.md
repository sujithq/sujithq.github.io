...
Validate that local production build-and-serve works end-to-end:

```powershell
npm run test:prod-local-health
```

Opt-in CI support is available in `.github/workflows/hugo.yml`:
- Set repository variable `RUN_PROD_LOCAL_HEALTH=true` to run it automatically on push/PR/schedule.
- Or run the workflow manually and set `run_prod_local_health=true` in `workflow_dispatch`.
